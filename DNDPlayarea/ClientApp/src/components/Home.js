...
export const Home = (props) => {
    ...
    const [width, setWidth] = useState(Math.min(500, window.innerWidth - 50));
    const [height, setHeight] = useState(Math.min(500, window.innerHeight - 50));
    const [color, setColor] = useState('red');


    const updateDimensions = () => {
        setWidth(Math.max(500, window.innerWidth - 50));
        setHeight(Math.max(500, window.innerHeight - 50));
    }
